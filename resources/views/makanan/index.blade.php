                            ...
                            @foreach ($datamakanan as $data)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <img src="{{ asset('images_makanan/' . $data->image_path) }}"
                                            alt="Restaurant Image" class="w-16 h-16 object-cover rounded-full">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">Rp
                                        {{ number_format($data->price, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ Str::limit($data->description, 20) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <form action="{{ route('makanan.destroy', $data->id) }}" method="POST"
                                            ...
                                            <button type="submit" onclick="confirmDelete()"
                                                data-pesan={{ $data->name }}
                                                class="text-red-500 btn-delete hover:text-red-700">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
